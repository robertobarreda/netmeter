 ...
 ***************************************************************************/
#ifndef NMGSTATISTICERROR_H
#define NMGSTATISTICERROR_H

#include <QString>

/*! \brief NMGGraphicsError is the class used by the Graphic Analysis plugin to emit possible
 * errors to the user. Errors are emited as modal dialog windows. */
class NMGStatisticError
{
	public:
		...
		* \param typeOfError Error type from supported error types. */
		inline NMGStatisticError ( QString title, QString error, NMGStatisticError::Error typeOfError )
		...
		inline void setErrorType ( Error errorType ) { type = errorType; }
